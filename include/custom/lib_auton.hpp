...
void fast_stack() {
  ...
  while(!bump.get_value()) {
      if(mtr_tilt.get_position() - tilt_offset < 1000) mtr_tilt.move(127);
      else if (mtr_tilt.get_position() - tilt_offset < 2000) mtr_tilt.move(127 - 15);
      else if (mtr_tilt.get_position() - tilt_offset < 3000) mtr_tilt.move(127 - 35);
      else if (mtr_tilt.get_position() - tilt_offset < 3500) mtr_tilt.move(127 - 55);
      else if (mtr_tilt.get_position() - tilt_offset < 3750) mtr_tilt.move(127 - 75);
      else mtr_tilt.move(127 - 110);
...
void auto_index(){
  ...
  pwr_intake(0);
  delay(500);
  while (limit_index.get_value() == false){
    mtr_rollR.move_velocity(300);
    mtr_rollL.move_velocity(300);
  }
